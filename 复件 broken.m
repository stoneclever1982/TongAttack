...
% 2 按轮生成s,c0,L；     
    for j=1:r
        ...
        %2.1.1解密cho{1,2} j 轮
        Decry('cho1.bmp', 'pcho1.bmp', 0, 3, j);
        Decry('cho2.bmp', 'pcho2.bmp', 0, 3, j);
       ...
       % 2.1.2 判断当前是第几轮，去掉前面 (j-1) 轮的密文，留下第 j 轮的中间图像。 
        ...
        pcho2=double(pcho2);
        s{j} =256-pcho1;
        s{j}(1)=N/2-1/2*mod(pcho1(1)+pcho2,N)-1/2;%第 j 轮所用 s{j}
        c0(j)=mod(double(pcho1(1))+s{j}(1),N);%第j轮所用c0(j)             
        
        %2.2 计算 L
        % 解密chosen{i} j 轮,并将解密结果写入img{i}
        for i=1:number
            filename = sprintf('chosen%d.bmp',j);
            filename2=sprintf('pchosen%d.bmp',j);
            Decry(filename,filename2, 0, 3, j);
